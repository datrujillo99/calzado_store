        ...
        <div class="col-md-8">
            <div class="card border-0 shadow-lg">
                <div class="card-header bg-warning text-dark p-4">
                    <h3 class="mb-0">
                        <i class="fas fa-edit"></i> Editar Zapato
                    </h3>
                </div>
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('calzados.update', $calzado->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('modelo') is-invalid @enderror" 
                                   id="modelo" name="modelo" placeholder="Ej: Air Max 270" value="{{ old('modelo', $calzado->modelo) }}" required>
                            @error('modelo')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('marca') is-invalid @enderror" 
                                   id="marca" name="marca" placeholder="Ej: Nike" value="{{ old('marca', $calzado->marca) }}" required>
                            @error('marca')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('categoria_id') is-invalid @enderror" 
                                ...
                                @foreach ($categorias as $categoria)
                                    <option value="{{ $categoria->id }}" @selected(old('categoria_id', $calzado->categoria_id) == $categoria->id)>
                                        {{ $categoria->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.01" class="form-control @error('precio') is-invalid @enderror" 
                                       id="precio" name="precio" placeholder="99.99" value="{{ old('precio', $calzado->precio) }}" required>
                                @error('precio')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" class="form-control @error('stock') is-invalid @enderror" 
                                       id="stock" name="stock" placeholder="0" value="{{ old('stock', $calzado->stock) }}" required>
                                @error('stock')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" step="0.5" class="form-control @error('talla') is-invalid @enderror" 
                                       id="talla" name="talla" placeholder="10.5" value="{{ old('talla', $calzado->talla) }}" required>
                                @error('talla')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="input-group">
                                    <input type="color" class="form-control form-control-color @error('color') is-invalid @enderror" 
                                           id="color" name="color" value="{{ old('color', $calzado->color) }}" required>
                                    <input type="text" class="form-control @error('color') is-invalid @enderror" 
                                           id="colorName" placeholder="Nombre del color" value="{{ old('color', $calzado->color) }}" disabled>
                                </div>
                                @error('color')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('descripcion') is-invalid @enderror" 
                                      ...
                                      placeholder="Escribe una descripción detallada del producto..." required>{{ old('descripcion', $calzado->descripcion) }}</textarea>
                            @error('descripcion')
                                <small class="text-danger">{{ $message }}</small>
                            @enderror
                        </div>
                        ...
                        @if ($calzado->imagen)
                            <div class="mb-3">
                                ...
                                <div class="card border-2">
                                    <img src="{{ asset('storage/' . $calzado->imagen) }}" alt="{{ $calzado->modelo }}" 
                                         class="card-img-top" style="height: 200px; object-fit: cover;">
                                    <div class="card-body">
                                        <small class="text-muted">{{ $calzado->modelo }}</small>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="mb-4">
                            <label for="imagen" class="form-label fw-bold">
                                <i class="fas fa-image"></i> Cambiar Imagen (Opcional)
                            </label>
                        ...
                        <div class="alert alert-info" role="alert">
                            ...
                            <ul class="mb-0 mt-2">
                                <li>Creado: {{ $calzado->created_at->format('d/m/Y H:i') }}</li>
                                <li>Última actualización: {{ $calzado->updated_at->format('d/m/Y H:i') }}</li>
                            </ul>
                        </div>
